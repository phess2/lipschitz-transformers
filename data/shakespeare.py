...
class TokenDataset:
    ...
    def __getitem__(self, idx):
        input_seq = jnp.array(
            self.data[idx : idx + self.context_length].astype(np.int32)
        )
        target_seq = jnp.array(
            self.data[idx + 1 : idx + self.context_length + 1].astype(np.int32)
        )
...
def cross_entropy_loss(model, w, inputs, targets):
    ...
    losses = -logits[batch_indices, seq_indices, targets] + jax.nn.logsumexp(
        logits, axis=-1
...
def load_shakespeare(
    context_length: int, batch_size: int, shuffle: bool = True
) -> Dict[str, Any]:
