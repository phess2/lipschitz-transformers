        ...
        while True:  # This allows for infinite iteration if repeat=True
            ...
            batch_indices = [
                indices[
                    i * self.batch_size : min(
                        (i + 1) * self.batch_size, len(self.dataset)
                    )
                ]
            ...
            def load_next_batch(batch_idx):
                ...
                return self._get_batch(
                    self._images, self._labels, batch_indices[batch_idx]
                )
...
def _load_cifar10_data(
    normalize=True, randomize_labels=False, dtype: jnp.dtype = jnp.float32
):
    """
    ...
    def load_batch(filename):
        ...
        return batch[b"data"].reshape(-1, 3, 32, 32).transpose(0, 2, 3, 1), np.array(
            batch[b"labels"]
        )
    ...
    if randomize_labels:
        randomize_labels = float(
            randomize_labels
        )  # for backward compatibility, convert True to 1.0
        rng = np.random.default_rng()
        n_reshuffled_indices = int(len(train_labels) * randomize_labels)
        reshuffled_indices = np.random.choice(
            np.arange(len(train_labels)), size=n_reshuffled_indices, replace=False
        )
        train_labels[reshuffled_indices] = rng.permutation(
            train_labels[reshuffled_indices]
        )

    return (
        train_images.astype(dtype),
        train_labels,
        test_images.astype(dtype),
        test_labels,
    )
...
def load_cifar10(
    ...
    train_images, train_labels, test_images, test_labels = _load_cifar10_data(
        normalize=normalize, randomize_labels=randomize_labels, dtype=dtype
    )
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=batch_size, shuffle=shuffle, repeat=repeat
    )
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False, repeat=repeat
    )
...
if __name__ == "__main__":
    ...
    for x_batch, y_batch in data["train_loader"]:
        ...
        print(
            ...
            [data["class_names"][int(label)] for label in y_batch[:5]],
        )
        break
