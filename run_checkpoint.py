...
# Update this to the npz file checkpoint! Works for Modula-style transformers, not NanoGPT
model_path = "path/to/your/model/checkpoint.npz"

def load_model(model_path, checkpoint=0):
    with np.load(model_path, allow_pickle=True) as data:
        args = data['args'].item()
        print(f"Loading checkpoint {checkpoint+1} of {len(data.keys()) - 3}")
...
if __name__ == "__main__":
    ...
    prompt = "O Transformio, my friend of infinite depth! Lipschitz continuity is desirable because "
