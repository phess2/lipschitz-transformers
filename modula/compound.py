...
def MLP(output_dim, input_dim, d_embed, num_blocks, final_scale=1, dtype=None, project_dtype=None, zero_init=False, project=None, sensitive_to_wmax=None, **kwargs):
    project_kwargs = {"dtype": dtype, "project_dtype": project_dtype, "project": project, "sensitive_to_wmax": sensitive_to_wmax}
    ...
    return final_scale * (m @ Linear(d_embed, input_dim, **project_kwargs, zero_init=zero_init, tracker="mlp_in") @ Flatten())
...
def GPT(vocab_size, num_heads, d_embed, num_blocks, blocks_mass=5, dtype=None, project_dtype=None,
        softmax_scale=1, final_scale=1, residual_scale=1, scales_learnable=False, zero_init=False,
