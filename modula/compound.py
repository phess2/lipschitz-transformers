...
def MLP(output_dim, input_dim, width, depth, project=None):
    m = Linear(output_dim, width, project=project, tracker="mlp_in") @ ReLU()
    for i in range(depth-2):
        m = m @ Linear(width, width, project=project, tracker=f"mlp_{i}") @ ReLU()
    return m @ Linear(width, input_dim, project=project, tracker="mlp_out")

def ManifoldMLP(output_dim, input_dim, width, depth, project=None):
...
def GPT(vocab_size, num_heads, d_embed, d_query, d_value, num_blocks, blocks_mass=5, softmax_scale=None, final_scale=None, residual_scale=None, scales_learnable=False, zero_init=False, project=None):
    embed = Embed(d_embed, vocab_size)
    embed.tare()

    blocks = Identity()
    for i in range(num_blocks):
        att = Attention(num_heads, d_embed, d_query, d_value, zero_init=zero_init, project=project, layer_idx=i)
...
def OrthogonalGPT(vocab_size, num_heads, d_embed, num_blocks, blocks_mass=5, softmax_scale=1.0, final_scale=1.0, residual_scale=1.0, scales_learnable=False, zero_init=False, project=None):
    embed = Embed(d_embed, vocab_size)
    embed.tare()
