...
for file_path in json_files:
    with open(file_path, 'r') as f:
        ...
        lr = result['parameters']['lr']

        # Get the last validation loss
        if 'val_losses' in result['results'] and len(result['results']['val_losses']) > 0:
            val_loss = min(result['results']['val_losses'])
            data.append((final_scale, softmax_scale, lr, val_loss))
        else:
...
lr_values = sorted(set(item[2] for item in data))

# Create a plot for each lr value
for lr in lr_values:
    # Create a grid for the heatmap
    loss_matrix = np.full((len(final_scales), len(softmax_scales)), np.nan)

    # Fill the grid with validation losses
    for final_scale, softmax_scale, lr_value, val_loss in data:
        if lr_value == lr:
    ...
    plt.savefig(f'{path}/val_loss_heatmap_lr={lr:.4f}.png', dpi=150, bbox_inches='tight')
    plt.show()

    print(f"Heatmap saved as {path / f'val_loss_heatmap_lr={lr:.4f}.png'}") 
